...
def generate_shap_inference_prompts(shap_prompt_glob, domain_knowledge_glob, output_prefix):
    ...
    and grouping by unique variant ID (e.g., LLM_shap_prompt_1_Throughput_Mbps.txt).
    """
    shap_prompt_files = sorted(glob.glob(shap_prompt_glob))
    domain_knowledge_files = sorted(glob.glob(domain_knowledge_glob))

    # Organize SHAP files by variant ID
    grouped_shap_files = {}
    for path in shap_prompt_files:
        name = Path(path).stem  # e.g., LLM_shap_prompt_1_Throughput_Mbps
        parts = name.split("_")
        variant_id = parts[3]  # Extract the number after 'LLM_shap_prompt'
        grouped_shap_files.setdefault(variant_id, []).append(path)

    for variant_id, shap_group in grouped_shap_files.items():
        domain_file = domain_knowledge_files[min(int(variant_id) - 1, len(domain_knowledge_files) - 1)]
        with open(domain_file, "r") as f:
            domain_knowledge = f.read()

        for shap_file in sorted(shap_group):
