...
env1_slice = sorted_envs[0]["env_name"].split("_")[-1].lower()  # e.g., "urlc2" or "embb1"
slice_type = "urllc" if "urllc" in env1_slice else "embb" if "embb" in env1_slice else "unknown"
...
for kpm in kpm_targets:
    ...
    if slice_type == "embb" and kpm != "Throughput_Mbps":
        ...
        print("⚠️ Unknown slice type, keeping all KPMs.")
    ...
    # Merge Scheduling_* into a single "Scheduling"
    result_raw = {feat: round(max(val, 0), 6) for feat, val in zip(features_list, extrapolated)}
    result = {}
    for feat, val in result_raw.items():
