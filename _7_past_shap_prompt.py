...
for variant_id in range(1, TOP_K + 1):
    ...
    for kpm_target in KPM_KEYS:
        prompt_lines = [
            ...
            "  - For b: β_b = Δγ",
            "",
            "**(ii) else if k_i = max(a, b)**:",
            "  - Compute: importance_shift_ratio Δγ= (a_curr / b_curr) / (a_past / b_past)",
            "  - For a: β_a = Δγ",
            "  - For b: β_b = 1 / Δγ",
            "",
            "**(iii) else if k_i = a / b or a + b or a - b or a * b **:",
            "  - Compute: importance_shift_ratio Δγ= (a_curr / ∂a) / (a_past / b_past)",
            "  - For a: β_a = (dk_i/ ∂a)_curr / (dk_i/ ∂a)_past",
            "  - For b: β_b = (dk_i/ ∂b)_curr / (dk_i/ ∂b)_past",
            "",
            "**(iv) otherwise, do not proceed to Step 2.",
            "",
            "---",
            ...
            "- Use the following **exact partial derivative formulas** — do NOT modify or reinterpret them.",
            "  - SNR_lin = 10^(SNR_dB/10)",
            "  - ∂C/∂PRB_num = log2(1+SNR_lin)",
            "  - ∂C/∂Avg_SNR_dB = PRB_num/(1+SNR_lin)",
            "- ❗ Do NOT use the incorrect version: PRB_num * SNR_lin / (1 + SNR_lin)"
            ...
            "---",
            "**Output Format**:",
            "Show me the detailed procedure for every step.",
            ...
            "Line 6: Step 4 – Repeat the process recursively if the feature has another normalized SHAP values. Show the process.",
            "Line 7: Results display – show me the values of β for all features in json format. Strictly follow below format and Only output final numeric values. DO NOT include expressions or equal signs.",
