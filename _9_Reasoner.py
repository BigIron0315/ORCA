...
def extract_clean_json(response: str):
    ...
    try:
        ...
        # 🐞 DEBUG: Print cleaned JSON before parsing
...
for i, prompt_file in enumerate(sorted(glob.glob(INPUT_TXT_GLOB)), start=1):
    ...
    # === Skip based on slice and KPM ===
    kpm_match = re.search(r"KPM is:\s*([^\n]+)", prompt)
    slice_match = re.search(r"### Reference Environment:\s*ORAN_log_(\w+)", prompt)

    if not kpm_match or not slice_match:
        print("⚠️ Missing KPM or slice info in the prompt, skipping.")
        continue

    target_kpm = kpm_match.group(1).strip()
    slice_type = slice_match.group(1).strip().lower()

    print(f"slice_type : {slice_type}, target_kpm : {target_kpm}")

    if "embb" in slice_type and target_kpm != "Throughput_Mbps":
        print(f"⏭️ Skipping {slice_type} with KPM {target_kpm} (Only Throughput_Mbps for eMBB)")
        continue
    elif "urllc" in slice_type and target_kpm != "Avg_Delay_ms":
        print(f"⏭️ Skipping {slice_type} with KPM {target_kpm} (Only Avg_Delay_ms for URLLC)")
        continue
    ...
    From the response between <BEGIN> and <END>
    Extract and show only final outputs in a json format.  
    Do not include any markdown, extra text, or code fences.
    ...
    """
    response = Query_GPT4(system_msg, json_extract_prompt, 0)
    "=============================================================================================="
    print(json_extract_prompt)
    "=============================================================================================="
    print(response)
